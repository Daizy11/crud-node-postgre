const stockScheme = require("../models/stockModels");

exports.getProduct = async (req, res) => {
  try {
    const products = await stockScheme.find(); // Retrieve all products from the database
    res.status(200).json({
      status: "success",
      data: products,
    });
...
exports.createProduct = async (req, res) => {
  ...
  try {
    const newProduct = await stockScheme.create(create); // Create a new product in the database
    res.status(201).json({
      status: "success",
      data: newProduct,
    });
...
exports.updateMetode = async (req, res) => {
  ...
  try {
    ...
    const [updatedRowCount, updatedProducts] = await stockScheme.update(
...
exports.deleteMetode = async (req, res) => {
  ...
  try {
    ...
    const product = await stockScheme.findByPk(productId);
